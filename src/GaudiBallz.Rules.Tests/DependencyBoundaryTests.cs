...
/// <c>GaudiBallz.Rules</c> depends on nothing.
///
...
public sealed class DependencyBoundaryTests
{
    ...
    {
        var project = XDocument.Load(Path.Combine(RepositoryRoot, "src", "GaudiBallz.Rules", "GaudiBallz.Rules.csproj"));
        ...
        Assert.True(
            declared.Length == 0,
            $"GaudiBallz.Rules must declare no dependencies, but declares: {string.Join(", ", declared)}. "
    ...
    public void Rules_uses_nothing_outside_the_base_class_library()
    {
        ...
        var path = Path.Combine(directory, "GaudiBallz.Rules.dll");

        Assert.True(File.Exists(path), $"Expected GaudiBallz.Rules.dll at {path}.");
        ...
        Assert.True(
            external.Length == 0,
            $"GaudiBallz.Rules must use only the base class library, but references: {string.Join(", ", external)}.");
    }
